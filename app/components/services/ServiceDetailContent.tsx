    ...
    <section className="border-y border-[#d4b499]/15 bg-white py-16 md:py-24">
      <div className="container mx-auto px-6 lg:px-24">
        <div className="mx-auto max-w-3xl">
          ...
          <h2 className="mb-3 font-serif text-3xl leading-tight tracking-tight text-[#4a3221] md:text-4xl">
            {editorial.title}
          </h2>
          {editorial.subtitle ? (
            <p className="mb-8 text-lg font-medium italic leading-relaxed text-[#7d5a44]">
              {editorial.subtitle}
            </p>
          ) : null}
          ...
          {editorial.relatedArticle ? (
            <div className="mt-10 rounded-[2rem] border border-[#d4b499]/30 bg-[#f7f0eb] p-8 md:p-10">
              <p className="mb-2 text-[10px] font-black uppercase tracking-[0.3em] text-[#a5846e]">
                En el blog
              </p>
              <Link
              ...
              <p className="mt-3 text-sm leading-relaxed text-[#7d5a44]">
                {editorial.relatedArticle.excerpt}
              </p>
            </div>
          ) : null}
        </div>
    ...
    <div className="min-h-screen bg-[#f7f0eb] font-sans text-[#4a3221]">
      <section className="relative pt-48 pb-24 text-center">
        ...
        <div className="relative z-10 container mx-auto px-6">
          <p className="mb-4 text-[10px] font-black uppercase tracking-[0.4em] text-[#d4b499]">
            {categoryTitle}
          </p>
          <h1 className="mb-6 font-serif text-5xl leading-none tracking-tight text-[#4a3221] md:text-7xl">
            {service.name}
          </h1>
        ...
        <div className="mx-auto max-w-2xl rounded-[2rem] border border-[#d4b499]/20 bg-white p-10 shadow-sm">
          ...
          <ul className="space-y-4">
            {rows.map((r, i) => (
              <li
                key={i}
                className="flex flex-col gap-1 border-b border-[#4a3221]/5 pb-4 last:border-0 sm:flex-row sm:justify-between sm:gap-8"
              >
                <span className="text-xs font-black uppercase tracking-widest text-[#7d5a44]">
                  {r.label}
                </span>
                <span className="text-sm font-semibold text-[#4a3221]">
                  {r.val}
                </span>
              </li>
            ))}
          ...
          <p className="mt-8 text-center text-sm text-[#7d5a44]">
            <Link
              href="/precios"
              className="border-b border-[#d4b499]/40 font-semibold hover:text-[#4a3221]"
            >
...
function FunnelHookSection({ data }: { data: Record<string, unknown> }) {
  const painPoints = data.painPoints as string[] | undefined;
  return (
    <section className="bg-[#4a3221] py-28 md:py-36">
      <div className="container mx-auto px-6 lg:px-24">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <h2 className="mb-8 font-serif text-4xl leading-tight text-[#f7f0eb] md:text-6xl">
            {String(data.title)}
          </h2>
          {data.body ? (
            <p className="mb-12 text-lg font-medium leading-relaxed text-[#f7f0eb]/75">
              {String(data.body)}
            </p>
          ) : null}
        </div>
        {painPoints && painPoints.length > 0 ? (
          <div className="mx-auto mb-12 grid max-w-4xl grid-cols-1 gap-4 md:grid-cols-2">
            {painPoints.map((point, i) => (
              ...
              >
                <span className="mt-0.5 shrink-0 font-serif text-xl text-[#d4b499]">
                  →
                </span>
                <p className="text-sm font-medium leading-relaxed text-[#f7f0eb]/80">
                  {point}
                </p>
              </div>
            ))}
        ...
        {data.solution ? (
          <p className="mx-auto max-w-2xl text-center font-serif text-xl italic text-[#d4b499] md:text-2xl">
...
function SocialProofSection({ data }: { data: Record<string, unknown> }) {
  const items = data.items as
    | { name: string; result: string; detail: string; image?: string }[]
    | undefined;
  return (
    <section className="py-28 md:py-36">
      <div className="container mx-auto px-6 lg:px-24">
        <div className="mx-auto mb-16 max-w-3xl text-center">
          ...
          <div className="mx-auto mb-6 h-px w-20 bg-[#d4b499]" />
          <h2 className="mb-6 font-serif text-4xl text-[#4a3221] md:text-5xl">
            {String(data.title)}
          </h2>
          {data.body ? (
            <p className="text-lg font-medium leading-relaxed text-[#7d5a44]">
              {String(data.body)}
            </p>
          ) : null}
        </div>
        {items && items.length > 0 ? (
          <div className="mx-auto grid max-w-5xl gap-8 md:grid-cols-3">
            {items.map((item, i) => (
              <div
                key={i}
                className="overflow-hidden rounded-[2rem] border border-[#d4b499]/20 bg-white shadow-sm"
              >
                {item.image ? (
                  <img
                    src={item.image}
                  ...
                  <div className="h-1 w-full bg-gradient-to-r from-[#d4b499] to-[#a5846e]" />
                )}
                <div className="p-8">
                  <p className="mb-2 text-[10px] font-black uppercase tracking-[0.3em] text-[#d4b499]">
                    {item.name}
                  </p>
                  <p className="mb-4 font-serif text-xl leading-snug text-[#4a3221]">
                    {item.result}
                  </p>
                  <p className="text-sm font-medium leading-relaxed text-[#7d5a44]">
                    {item.detail}
                  </p>
                </div>
              </div>
    ...
    <section className="bg-white py-32">
      <div className="container mx-auto px-6 lg:px-24">
        <div className="mx-auto mb-24 max-w-3xl text-center">
          <h2 className="mb-6 font-serif text-4xl text-[#4a3221] md:text-6xl">
            {String(data.title)}
          </h2>
          {data.subtitle ? (
            <p className="font-medium text-[#7d5a44] opacity-80">
          ...
          {items.map((item, i) => (
            <div key={i} className="group">
              <div className="mb-6 flex items-end justify-between gap-4">
                <h3 className="text-[11px] font-black uppercase tracking-[0.4em] text-[#4a3221]">
                  {item.title}
                </h3>
                <div className="mb-1.5 flex-grow border-b border-dotted border-[#d4b499]/30" />
                <ServiceIcon
                  name={item.icon}
                  className="h-5 w-5 shrink-0 text-[#d4b499]"
                />
              </div>
              <p className="pr-8 text-sm font-medium leading-relaxed text-[#7d5a44]">
                {item.description}
              </p>
            </div>
          ))}
...
function ProblemLikeSection({
  ...
  const causes = (data.causes ?? data.signs) as
    | { title: string; description: string }[]
    | undefined;
  const approach = data.approach as
    ...
    <section className="container mx-auto px-6 py-24 lg:px-24">
      ...
      <div className="mx-auto grid max-w-5xl gap-10 md:grid-cols-2">
        {causes.map((c, i) => (
          <div
            key={i}
            className="rounded-3xl border border-[#d4b499]/15 bg-white/60 p-8 shadow-sm"
          >
            <h3 className="mb-3 font-serif text-xl text-[#4a3221]">
              {c.title}
            </h3>
            <p className="text-sm font-medium leading-relaxed text-[#7d5a44]">
              {c.description}
...
function ExpectationsSection({ data }: { data: Record<string, unknown> }) {
  const timeline = data.timeline as
    | { phase: string; icon: string; description: string }[]
    | undefined;
      ...
      <section className="bg-[#fbf6f3] py-24">
        <div className="container mx-auto px-6 lg:px-24">
          <div className="mb-16 text-center">
            <h2 className="font-serif text-4xl text-[#4a3221] md:text-5xl">
              {String(data.title)}
            </h2>
            {data.subtitle ? (
              <p className="mt-4 text-[#7d5a44]">{String(data.subtitle)}</p>
            ) : null}
          </div>
          <div className="mx-auto grid max-w-6xl gap-10 md:grid-cols-2 lg:grid-cols-4">
            {timeline.map((t, i) => (
              <div
                key={i}
                className="rounded-[2rem] border border-[#d4b499]/20 bg-white p-8"
              >
                <ServiceIcon
                  name={t.icon}
                  className="mb-4 h-8 w-8 text-[#d4b499]"
                />
                <p className="mb-2 text-[10px] font-black uppercase tracking-[0.3em] text-[#d4b499]">
                  {t.phase}
                </p>
  ...
  const immediate = data.immediate as
    | { title: string; description: string }
    | undefined;
  const shortTerm = data.shortTerm as
    | { title: string; description: string }
    | undefined;
  const longTerm = data.longTerm as
    | { title: string; description: string }
    | undefined;
          ...
          <div className="mx-auto grid max-w-5xl gap-10 md:grid-cols-3">
            {[immediate, shortTerm, longTerm].map((block, i) => (
              <div
                ...
                className="rounded-[2rem] border border-[#d4b499]/20 bg-white p-8"
              >
                <h3 className="mb-4 font-serif text-xl text-[#4a3221]">
                  {block.title}
                </h3>
                <p className="text-sm font-medium leading-relaxed text-[#7d5a44]">
                  {block.description}
...
function CareListsSection({
  ...
  const doList = (mode === "care" ? data.doList : data.recommended) as
    | { title: string; description: string }[]
    | undefined;
  const dontList = (mode === "care" ? data.dontList : data.avoid) as
    | { title: string; description: string }[]
    | undefined;
      ...
      <div className="container mx-auto px-6 lg:px-24">
        ...
        <div className="mx-auto grid max-w-5xl gap-12 lg:grid-cols-2">
          ...
          {dontList && dontList.length > 0 ? (
            <div>
              <p className="mb-6 text-[10px] font-black uppercase tracking-[0.4em] text-[#a5846e]">
                Evitar
              </p>
              <ul className="space-y-6">
                {dontList.map((item, i) => (
      ...
      <div className="container mx-auto px-6 lg:px-24">
        ...
        <div className="mx-auto grid max-w-5xl gap-8 md:grid-cols-2">
          {items.map((item, i) => (
            <div
              key={i}
              className="flex gap-4 rounded-3xl border border-[#d4b499]/20 bg-white p-6"
            >
              <ServiceIcon
                name={item.icon}
                className="mt-1 h-6 w-6 shrink-0 text-[#d4b499]"
              />
              <div>
...
function ComparisonSection({ data }: { data: Record<string, unknown> }) {
  const individual = data.individual as
    | { title: string; items: string[] }
    | undefined;
  const combined = data.combined as
    | { title: string; items: string[] }
    | undefined;
      ...
      <div className="container mx-auto px-6 lg:px-24">
        ...
        <div className="mx-auto grid max-w-5xl gap-10 md:grid-cols-2">
          {[individual, combined].map((col, idx) => (
            <div
              key={idx}
              className={`rounded-[2rem] border p-10 ${idx === 1 ? "border-[#d4b499] bg-white shadow-lg" : "border-[#d4b499]/20 bg-white/80"}`}
            >
              <h3 className="mb-8 font-serif text-2xl text-[#4a3221]">
                {col.title}
              </h3>
              <ul className="space-y-4">
                {col.items.map((line, i) => (
                  <li
                    key={i}
                    className="flex gap-3 text-sm font-medium text-[#7d5a44]"
                  >
...
function AftercareSection({ data }: { data: Record<string, unknown> }) {
  const first24h = data.first24h as
    | { title: string; description: string }[]
    | undefined;
  const firstWeek = data.firstWeek as
    | { title: string; description: string }[]
    | undefined;
...
function ProcessAndDetailsSection({
  ...
  return (
    <section
      id="detalles"
      className="container mx-auto scroll-mt-28 px-6 py-32 lg:px-24"
    >
      <div className="grid items-center gap-20 lg:grid-cols-12">
        ...
        <div className="lg:col-span-6">
          ...
          <div className="mb-10 space-y-6 border-t border-[#4a3221]/10 pt-6">
            <p className="text-[10px] font-black uppercase tracking-[0.4em] text-[#d4b499]">
              ...
              {process.duration ? (
                <li className="flex items-baseline justify-between gap-4">
                  ...
                  <span className="text-sm font-bold text-[#4a3221]">
                    {String(process.duration)}{" "}
                    {process.durationUnit ? String(process.durationUnit) : ""}
                  </span>
                </li>
              ) : null}
              {rows.map((r, i) => (
                ...
                >
                  ...
                  <div className="mb-1 flex-grow border-b border-dotted border-[#d4b499]/20" />
                  <span className="max-w-[55%] text-right text-sm font-bold text-[#4a3221]">
                    {r.val}
                  </span>
                </li>
              ))}
          ...
          {includes && includes.length > 0 ? (
            <div className="mb-12 rounded-[2.5rem] border border-[#d4b499]/20 bg-white/50 p-10">
              <p className="mb-6 text-[10px] font-black uppercase tracking-[0.4em] text-[#4a3221]">
                ¿Qué incluye?
              </p>
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                {includes.map((t, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-3 text-sm font-bold text-[#7d5a44]"
                  >
                    <CheckCircle2
                      size={14}
                      className="shrink-0 text-[#d4b499]"
                    />{" "}
                    {t}
                  </div>
                ))}
      ...
      {steps && steps.length > 0 ? (
        <div className="mt-32 rounded-[3rem] bg-[#4a3221] px-6 py-20 text-[#f7f0eb] lg:px-24">
          <div className="mb-16 text-center">
            <span className="mb-6 block text-[10px] font-black uppercase tracking-[0.4em] text-[#d4b499]">
              Metodología
            </span>
            <h3 className="font-serif text-4xl tracking-tighter uppercase md:text-6xl">
              {String(process.title)}
            </h3>
            {process.subtitle ? (
              <p className="mt-4 text-sm font-medium opacity-70">
                {String(process.subtitle)}
              </p>
            ) : null}
          </div>
          <div className="grid grid-cols-1 gap-16 md:grid-cols-2 lg:grid-cols-4">
            {steps.map((step, i) => (
              <div
                key={i}
                className="group relative border-t border-white/10 pt-12 transition-colors hover:border-[#d4b499]"
              >
                <span className="absolute -top-6 left-0 font-serif text-5xl italic text-[#d4b499] opacity-30">
                  {String(step.number).padStart(2, "0")}
                </span>
                <h4 className="mb-4 font-serif text-xl uppercase tracking-tight transition-colors group-hover:text-[#d4b499]">
                  {step.title}
                </h4>
...
function DetailsWithoutProcess({
  ...
  return (
    <section
      id="detalles"
      className="container mx-auto scroll-mt-28 px-6 py-24 lg:px-24"
    >
      <div className="mx-auto max-w-3xl rounded-[2.5rem] border border-[#d4b499]/20 bg-white p-12 shadow-sm">
        ...
        <ul className="mb-10 space-y-4">
          {rows.map((r, i) => (
            <li
              key={i}
              className="flex flex-col gap-1 border-b border-[#4a3221]/5 pb-4 last:border-0 sm:flex-row sm:justify-between"
            >
              <span className="text-xs font-black uppercase tracking-widest text-[#7d5a44]">
                {r.label}
              </span>
              <span className="text-sm font-bold text-[#4a3221] sm:max-w-[60%] sm:text-right">
                {r.val}
              </span>
            </li>
          ))}
  ...
  return (
    <section className="relative overflow-hidden bg-[#f7f0eb] py-40 text-center">
      <div className="container relative z-10 mx-auto px-6">
        <h2 className="mb-6 font-serif text-5xl leading-none tracking-tighter text-[#4a3221] md:text-7xl">
          {String(data.title)}
        </h2>
        {data.subtitle ? (
          <p className="mx-auto mb-10 max-w-2xl text-lg font-medium text-[#7d5a44]">
            {String(data.subtitle)}
          </p>
        ) : null}
        {data.tip ? (
          <p className="mb-10 text-sm italic text-[#d4b499]">
            {String(data.tip)}
          </p>
        ) : null}
        {data.promise ? (
          <p className="mb-10 text-sm font-bold text-[#4a3221]">
            {String(data.promise)}
          </p>
        ) : null}
        {data.highlight ? (
          <p className="mb-10 font-serif text-xl text-[#d4b499]">
...
function renderSection(
  ...
  switch (key) {
    case "funnelHook":
      return <FunnelHookSection key={key} data={value} />;
    case "socialProof":
      return <SocialProofSection key={key} data={value} />;
    case "benefits":
    ...
    case "understandingSection":
      return (
        <ProblemLikeSection key={key} data={value} variant="understanding" />
      );
    case "expectations":
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#f7f0eb] font-sans text-[#4a3221] selection:bg-[#d4b499] selection:text-white">
      <section className="relative flex min-h-[55vh] items-center justify-center pb-32 pt-48 text-center">
        ...
        <div className="relative z-10 container mx-auto px-6">
          ...
          <h1 className="font-serif text-5xl leading-none tracking-tight text-[#4a3221] md:text-[8.5rem]">
            {hero.title}
            <span className="font-script -mt-2 block text-4xl lowercase text-[#d4b499] md:-mt-8 md:text-9xl">
              {hero.titleHighlight}
            </span>
          </h1>
      ...
      {!sectionKeys.includes("process") ? (
        <DetailsWithoutProcess service={service} />
      ) : null}

      {sectionKeys.map((key) =>
        renderSection(key, page[key], pair, processImg),
      )}
